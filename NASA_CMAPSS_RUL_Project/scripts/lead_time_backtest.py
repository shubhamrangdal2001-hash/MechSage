...
Uses the already-trained models from NASA_CMAPSS_RUL_Project/models/:
...
Writes report to NASA_CMAPSS_RUL_Project/reports/lead_time_backtest_FD001.md.

Usage (from NASA_CMAPSS_RUL_Project directory OR from MechSage root):
    python NASA_CMAPSS_RUL_Project/scripts/lead_time_backtest.py
"""
...
# ---------------------------------------------------------------------------
# Path setup: allow running from project root or from NASA_CMAPSS_RUL_Project
# ---------------------------------------------------------------------------
_SCRIPT_DIR = Path(__file__).resolve().parent
_CMAPSS_ROOT = _SCRIPT_DIR.parent  # NASA_CMAPSS_RUL_Project/
...
_REPORTS_DIR = _CMAPSS_ROOT / "reports"
_DATASET_ID = "FD001"
_RUL_CLIP = 125
    ...
    for p in (anomaly_path, rul_path):
        if not p.exists():
            raise FileNotFoundError(
                ...
                "Run the NASA_CMAPSS_RUL_Project pipeline first to generate models."
            )
...
def run_backtest() -> None:
    ...
    mean_lead_status = "✅ PASS" if mean_lead >= 20 else "⚠️ REVIEW"
...
|---|---|---|---|
| **Mean Lead Time** | {mean_lead:.1f} cycles | ≥ 20 cycles | {mean_lead_status} |
| **Median Lead Time** | {median_lead:.1f} cycles | — | ✅ MEASURED |
| **P10 Lead Time (worst 10%)** | {p10_lead:.1f} cycles | — | ✅ MEASURED |
| **P90 Lead Time (best 90%)** | {p90_lead:.1f} cycles | — | ✅ MEASURED |
| **Early-Warning Rate (RUL > {_EARLY_WARNING_CYCLES} cycles)** | {detected_early}/{len(units)} = {early_warning_rate:.1f}% | ≥ 70% | {early_rate_status} |
| **Detection Rate (any alert)** | {detected_any}/{len(units)} = {detected_any/len(units)*100:.1f}% | — | ✅ MEASURED |
...
- A **lead time ≥ 20 cycles** means maintenance can be proactively scheduled before failure.
...
*Report generated by `NASA_CMAPSS_RUL_Project/scripts/lead_time_backtest.py` as part of Sprint 2.*
"""
